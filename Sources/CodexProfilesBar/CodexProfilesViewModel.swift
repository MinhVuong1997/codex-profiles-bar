...
final class CodexProfilesViewModel: NSObject, ObservableObject {
    ...
    private var activeProfileRefreshTask: Task<Void, Never>?
    ...
    deinit {
        ...
        activeProfileRefreshTask?.cancel()
    ...
    func refresh(trigger: RefreshTrigger = .manual) async {
        await performRefresh(trigger: trigger, scope: .allProfiles)
    }

    private func refreshActiveProfile(trigger: RefreshTrigger = .automatic) async {
        await performRefresh(trigger: trigger, scope: .activeProfile)
    }
    ...
    @discardableResult
    func addProfile() async -> Bool {
        guard !isAddingProfile else { return false }
        isAddingProfile = true
        isLoading = true
        isCodexLoginCancelled = false
        let shouldPreserveCurrent = hasUnsavedCurrentProfile
        defer {
        ...
        do {
            ...
            let body = shouldPreserveCurrent
                ...
                : "Signed in with Codex and stored the new profile."
            showBanner(title: "Profile added", body: body, tone: .success)
            await refresh(trigger: .mutation)
            return true
        } catch {
            if isCodexLoginCancelled {
                showBanner(title: "Add profile cancelled", body: "Codex login was stopped before a new profile was saved.", tone: .warning)
            } else {
                showBanner(title: "Add profile failed", body: error.localizedDescription, tone: .error)
            }
            return false
        }
    }
    ...
    private func configureAutoRefreshLoop() {
        configureActiveProfileRefreshLoop()
        configureAllProfilesAutoRefreshLoop()
    }

    private func configureActiveProfileRefreshLoop() {
        activeProfileRefreshTask?.cancel()
        guard !usesProxyDrivenRefresh else {
            activeProfileRefreshTask = nil
            return
        }

        activeProfileRefreshTask = Task { [weak self] in
            while let self, !Task.isCancelled {
                try? await Task.sleep(for: activeProfileRefreshInterval)
                guard !Task.isCancelled else { break }
                await self.refreshActiveProfile(trigger: .automatic)
            }
        }
    }

    private func configureAllProfilesAutoRefreshLoop() {
        autoRefreshTask?.cancel()
    ...
    private func scheduleProxyDrivenRefresh() {
        ...
        proxyRefreshTask = Task { @MainActor [weak self] in
            ...
            guard !Task.isCancelled else { return }
            await refreshActiveProfile(trigger: .automatic)
        }
    }
    ...
    private func runCodexLogin() async throws {
        ...
        try await Task.detached(priority: .userInitiated) {
            ...
            guard process.terminationStatus == 0 else {
                throw CodexProfilesError.commandFailed(output.isEmpty ? "Codex login was cancelled or failed." : output)
            }
        }.value
    }

    private func checkCodexLoginCancellation() throws {
        if isCodexLoginCancelled {
            throw CodexProfilesError.commandFailed("Codex login was cancelled.")
        }
    }
