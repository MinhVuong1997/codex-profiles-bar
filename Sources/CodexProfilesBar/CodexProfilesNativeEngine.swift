...
actor CodexProfilesNativeEngine {
    ...
    func setModelProxyRuntimeModel(contextWindow: Int?, autoCompactTokenLimit: Int?) throws -> ModelProxyRuntimeModel {
        ...
        let withScope = replaceRemoveOrAppendConfigLine(
            contents: updated,
            key: "model_auto_compact_token_limit_scope",
            line: autoCompactTokenLimit.map { _ in "model_auto_compact_token_limit_scope = \"total\"" }
        )
        try writeAtomicPrivate(data: Data(withScope.utf8).appendingNewline(), to: paths.config)
...
private extension CodexProfilesNativeEngine {
    ...
    func replaceRemoveOrAppendConfigLine(contents: String, key: String, line: String?) -> String {
        ...
        var firstSectionIndex = lines.firstIndex(where: { parseSectionHeader($0) != nil }) ?? lines.count
        ...
        while firstSectionIndex > 0 && lines[firstSectionIndex - 1].trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            lines.remove(at: firstSectionIndex - 1)
            firstSectionIndex -= 1
        }
        var insertedLines = [line, ""]
        if firstSectionIndex > 0 {
            insertedLines.insert("", at: 0)
        }
